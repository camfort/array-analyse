...
values and produces a specification which contains a one-dimensional
region. The 


$\oplus$ and $\odot$ respectively (defined in Definition~\ref{}
and Definition~\ref{} below).

The proceeds by converting each span
into a specification by converting ea
 is mapped to a specification,
and then summing specifications. This relies on product and sum
operations lifted to specification syntax (which is more than just
building regions out of \term{*} and \term{+}). 
...
\begin{equation*}
{\Large{\bullet}}_{i \in \{1, \ldots, n\}} \, \mathsf{toSpec}(\vect{x}^L_i, \vect{x}^U_i)
\end{equation*}
...
The first two columns of Figure \ref{fig:tickAssign} shows the number of potential and actual stencils and their rate as a percentage of the total number of assignment statements in the code.
