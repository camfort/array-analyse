...
%\begin{defn}{(\emph{Offsets vector}, $\mathcal{P}(\zinf^N)$)}
%  An $N$ dimensional \emph{offset space vector} is a Cartesian product of offset
%  spaces denoted with $\textit{offs}_1 \times \cdots \times
%  \textit{offs}_N$.
%  The function $\pi_i : \mathcal{P}(\zinf{})^{\mkern-19mu{N}} \to
%  \mathcal{P}(\zinf{})$ retrieves the $i^{th}$ offset space.%
%
...
\todo{We can move this explanation.}
...
\begin{proof}
  ...
  \region{N} is a subset of $\mathcal{P}{(\zinf{}^{\mkern0mu{N}})}$ since all
...
interval $[-1, 1]^N$ which is a subset of \region{N}. 
\end{defn}
...
%
\begin{proof}
\todo{Not sure what we are going to prove here? That it is a metric
space or something?}
\end{proof}

\todo{We used to call the below property contiguity, and I think this is a natural
notion- does ``close neighbour'' have some standard use or is
it okay to call this contiguity?}
...
\todo{Not sure how to rewrite the following as I have a bit of hard
time parsing it.}
...
%
  ...
  distance in $P$.
\end{defn}
\todo{Ah, I think I am starting to understand, but at 
$\mathit{ES}_{P,1}$ then we end up saying there exists a 
$p'$ in $\textit{ES}_{P,0}$, but we don't calculate that set...
