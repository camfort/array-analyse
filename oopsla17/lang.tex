...
\end{minted}
%%
If a dimension contains only absolute index terms then it is left
unspecified.
...
of the origin, corresponding to those array computations classified
as \textsf{straddle} in the previous section.  For example, the following is a
...
consistent with both $r$ and $s$ simultaneously.
 ...
 consistent with one of $r$ or $s$, or both.
...
and so ranges over the assignments to \term{x},
\term{y}, and \term{z}. Each index in the code is consistent
with both parts of the specification, \eg{}, \texttt{a(i-1, j+1)}
...
The \texttt{readOnce} modifier enforces that no subscript appears more
...
exactly once. We opt for a more informative and easily understood name
\texttt{readOnce}.
...
\paragraph{Relativisation}

The data access pattern of a stencil is understood relative to the
subscript on the left-hand side. If the left-hand side of a stencil
has a neighbourhood subscript with non-zero offsets, then the
right-hand side is understood relative to this shifted origin. For example:
%%
\begin{minted}{fortran}
do i = 1, n
   != stencil (backward(depth=2, dim=1, nonpointed))*(pointed(dim=2)) :: b
   a(i+1, j) = b(i, j) + b(i-1, j)
\end{minted}
%%
Note that the specification for dimension 1 is to a depth of 2, since
the left-hand side is shifted forward by 1.

\paragraph{Access specifications}

Specifications can also be given to array computations which are not
stencils (\ie{}, do not have a left-hand side which is an array
subscript). For example, for a reduction:
%%
\begin{minted}{fortran}
do i = 1, n
  != access pointed(dim=1) :: a
  r = max(a(i), r)
end do
\end{minted}
%%
The main difference between \fortran{stencil} and \fortran{access} is
that there is no left-hand side with which to check consistency or
relativise against.
