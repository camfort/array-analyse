...
checking algorithms in \Cref{sec:algorithms}; (3) justifies the equational
theory for specifications; (4) is used to optimise
...
and properties, which we exploit. We first set up the domain of the
model (\S\ref{sec:domain}), using it to define a semantics for our
specification language (\S\ref{sec:semantics}) and then as the target
for an interpretation of the sytnax of array subscripts in code
(\S\ref{sec:fromcode}).  We state various results in this section, for
which the proofs are provided in \Cref{sec:proofs}.
...
\begin{defn}[Index scheme]
  ...
  $S$, we refer to $\pi_i(S)$ as the \emph{$i^{th}$ component} of $S$. We assume
...
\begin{defn}[Interval scheme]\label{def:interval-scheme}
  ...
  product) of intervals on \bz{} (as in \Cref{defn:my-interval}),
  denoted by the set $\textit{Interval}^N$ for a product of $N$
  intervals.
\end{defn}
...
%
  ...
  \end{align*}
%
  \eg{}, $\mathsf{lower}$ is an injection into $\mathsf{Approx}$, of
  type $\mathsf{lower} : a \to \mathsf{Approx} \; a$.
...
\begin{align*}
  ...
  %
   \interp{\texttt{\textcap{r} + \textcap{s}}}_N & =
    \interp{\textcap{r}}_N \cup \interp{\textcap{s}}_N
\\
  \interp{\texttt{\textcap{r} * \textcap{s}}}_N & =
   \interp{\textcap{r}}_N \cap \interp{\textcap{s}}_N \\
%%
...
The intermediate interpretation $\interp{-}$ of
\Cref{subfig:region-model} models region constants. This is lifted by
...
regions. Thus the syntax of regions is modelled by members of
$\region{N}$.
...
the static analysis preceding checking and inference.
...
Subscripts schemes do not enjoy the same closure property as the intersection of
...
Union is not as flexible as intersection; it is not closed over
indexing schemes, only on indexing schemes which are equal in all
components but one (\Cref{lem:vector-union}). For this reason, we
represent sets of indexing schemes as unevaluated union terms, or
\emph{union normal form}.  Since the model forms a distributive
lattice as established in \Cref{prop:region-lattice}, intersections
can be pushed inwards using the distributive law leaving only unions
at the outer-level. We further exploit associativity of union to put
unions into a cons-tree effectively representing union normal form as
a non-empty list.  We do not, however, attempt to canonicalise the
representation. However, at the end of the inference procedure,
\Cref{lem:vector-union} and index scheme subset are used to reduce the
size of union normal form as a way of simplifying specifications.
