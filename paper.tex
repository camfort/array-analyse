...
algorithms for our language (\S~\ref{sec:analysis}), derived from
a denotational model for the language (\S~\ref{sec:semantics});
...
\S~\ref{sec:syntax} outlines the syntax.
 \S~\ref{sec:eqs} provides an equational
theory for specifications and a notion of
sub-specification.  \S~\ref{sec:semantics}
...
checking, and synthesis algorithms (\S~\ref{sec:analysis}); (2)
...
to guide correct implementations. The model domain is over sets of vectors which we call \emph{index
  schemes}, which we introduce first before the model in \S~\ref{subsec:model}.
...
\S~\ref{sec:analysis}.% with additional information.
...
specifications (\S~\ref{subsec:inference}), for checking code
against specifications (\S~\ref{subsec:checking}). These utilise the semantic model of
\S~\ref{sec:semantics}.
...
\S~\ref{sec:semantics} again aids this process.  Checking
...
One way implementation may deviate from the model is elimination of all $\infty$
...
\S~\ref{sec:lang}. This introduces a new notion of approximation
beyond what is given by the syntactic theory of sub-specifications
$<:$ in \S~\ref{sec:eqs}. Consistency leads to a notion of
...
\S~\ref{subsec:inference}. Each potential statement for which we
