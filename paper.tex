...
(which we denote \specDNF{}). The algebra mirrors the shape
of the region syntax with additive and multiplicative binary operators
$\oplus$ and $\odot$ and a constructor for \specDNF{}:
%%
\begin{align*}
...
values (one-dimensional) and constructs a specification (in the
\textit{spec} syntax) which contains a one-dimensional
region. An entire span is convereted then into a $n$-dimensional
specification by applying $\mathsf{toSpec}$ to each
pair of bounds in a span, per dimension, and multiplying these
specifications by $\odot$. The resulting specifications for
each span are then summed by by $\oplus$, that is:
\begin{equation*}
\textit{spec}(M) = 
...
\begin{definition}[DNF specifications]
Let $r_1, ..., r_n$ range over elements of the \textit{region}
syntax (see Figure~\ref{fig:syntax}, p.~\pageref{fig:syntax})
The domain of \specDNF{} specifications
in disjunctive normal form ranges over terms of the form:
%%
(r_1 \term{*} \ldots \term{*} r_n)\term{+} \ldots 
\term{+} (r_1 \term{*} \ldots \term{*} r_n)
%%
\end{definition}
...
\begin{definition} The $\odot$ operation is defined:
\begin{align*}
\tySum{r_1} \odot \tySum{r_2} = 
 \bplus_{(\tyProd{s_1}, \tyProd{s_2}) \in (\tySum{r_1} \times
   \tySum{r_2})} (\tyProd{s_1} \; \term{*} \; \tyProd{s_2})
%\begin{cases}%
%\end{cases}
\end{align*}
\end{definition}
