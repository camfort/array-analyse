...
Typographically, \textcap{italic monotype font} is used within specifications
...
\begin{proof}
  ...
  \longv{%
    ...
    Proof of the second inequality:
    \begin{align*}
      \stencil{R}{$n$}{$k$}{\textcap{nonpointed}}
        & \preceq \stencil{R}{$n$}{$k$}{\texttt{nonpointed}}
                  \;\texttt{+}\;
                  \stencil[s]{p}{$n$}{}{}
        & (\textbf{Combined}) \\
...
\begin{defn}{(Extended origin)}
  In the $N$ dimensional region space, ${(\interv{-1}{1}{\textsf{true}})}^N$ is
  said to be the extended origin.
\end{defn}

\begin{defn}{(Hearing)}
  A point, $p$, in \region{N} is said to \emph{talk to} the extended origin if
%
  \begin{enumerate}
    \item $p$ is in the extended origin;
    \item there are distinct indices $i$ and $j$ (between $1$ and $N$) such that
      $p_i$ is $-1$ to $1$ and there is a $p'$ that talks to the extended origin
      such that $p_j$ is ${p'}_j - 1$ or ${p'}_j + 1$ and for all other indices
      $k$, $p_k$ is ${p'}_k$;
    \item all immediate neighbours of $p$ that are no farther away from the
      extended origin than $p$ can talk to the extended origin.
  \end{enumerate}

  A set of points talk to the extended origin if all points in the set talk to
  the extended origin.
\end{defn}
