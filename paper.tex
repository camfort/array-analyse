...
\begin{abstract}
  ...
  precise specifications can be given to roughly half of the
  identifiable stencil computations, and that the vast majority of
  ...
  procedures, our implementation, and our evaluation study.
\end{abstract}
...
along with a denotational model (Section~\ref{sec:semantics});
...
number greater than 0). Each receives a dimension identifier $d$ (also a
...
A specification is first synthesised into its model. Then the occurrences of an
...
\end{dmath*}
%
\item Now the $cons$ predicate is applied with this model alongside the
  assignment on line three and array $b$. For brevity the assignment is
  referred to as $L3$ and the set of unifiers defined in the previous step as
  $m$.
...
%
  This condition holds as no access pattern is repeated in $L3$ by simple
  inspection.
%
...
\item To evaluate the two predicates above, we need indexing schemes for the
  array in question. This is obtained by the $analyse$ function from
  Subsection~\ref{subsec:inference}.
...
\end{minted}
%
The first four steps proceed as before. In the fifth step, the result
of $\mathit{analyse}(L3',b)$ will have an extra term $\vtwoh{2}{2}$. In the
