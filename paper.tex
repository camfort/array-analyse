...
We provide a verification tool which can check the correctness of stencil code
against its specification, infer such specifications from bare source, and
synthese inferred specifications as comments and insert them into the source
...
\item The primary use case is that a programmer writes a specification first,
  then the corresponding source code and the tool checks conformance between
  the two. This specify-and-check approach reduces the testing effort and
...
\item A specification concisely captures the access pattern and turns it into
  documentation. Inference algorithm efficiently produces specifications with
  no programmer intervention, while synthesis mechanism inserts the
  specifications at appropriate places in the source code automatically.
%
...
\begin{definition}[Consistency, informal]
  ...
  is an $n$-dimensional shape crossing the origin with respect to the induction
...
\begin{definition}
  ...
  index. Throughout $u, v, w$ range over
...
$(i, j+1)$ can be instantiated to $(1, j+1)$.
...
\dfrac{R <:^r S}{R <: \term{atMost} \, S}\trule{\textsc{shrink}} \quad
\dfrac{R <:^r S}{S <: \term{atLeast} \, R}\trule{\textsc{grow}} \quad
 \\[1.5em]
%%
...
Rules $(1)$ and $(*)$ extend the \consAName{}
predicate to $\consAName{}_m \subseteq
