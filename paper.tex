...
\begin{abstract}
  ...
  \emph{stencil computations}-- a common idiom in numerical code.  Our
  conjecture is that the vast majority of stencil computations have a
  simple, regular, static shape that can be captured by a simple
  specification language. We propose a language for this, along with
  an inference and checking procedure. We test our conjecture against
  a corpus of numerical code, and show that we can give static
  specifications to $X\%$ of identifiable stencil computations, with a
  mean ratio of $Y:Z$ specification-to-program code.  This paper
  details our stencil specification language, its static semantics,
  inference, checking, and program synthesis procedures, our
  implementation, and our evaluation study.
\end{abstract}
