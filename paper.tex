...
analytical solutions~\cite{farrell2010automated}). Typically such
...
Laplace example stencil is specified in our language as:
%
...
\item The primary use case is that a programmer writes a specification
  ...
  future-proofs against bugs introduced during refactoring and maintenance.
%
...
\item We detail checking and inference algorithms for our
  ...
  denotational model for the specification language
  (\cref{sec:semantics});
...
\item We give a verification case study of a
  stencil computation commonly used in scientific computing,
...
%arrays $\bar{v}$ that are simultaneously permitted and expected within
...
\Cref{sec:semantics} and algorithmically described in
...
origin (which represents a collection of indices which are just 
induction variables). Consistency is then a
...
regions disjunctively and conjunctively.

The conjunction of two regions $r \term{*} s$ means that any indices
in the specified code must be consistent with both $r$ and $s$
simultaneously. Dually, for every pair of components in the regions
...
Index schemes abstract array subscripts.
Our model consists of index schemes explaining the range of possible
...
induction variables $I$, mapping source language expressions to
$\domainVal$ values:
%%
\begin{align*}
\schemeOne(e)
...
index, $\schemeN$ maps it to an index scheme.
\end{definition}
...
\begin{example}
Consider the following array subscript to index schemes
conversions:
%
\begin{align*}
\begin{array}{ll}
...
$n$-dimensional index schemes. The interpretation is overloaded on
...
% r*s means indices must be consistent simultaneously with both r and s
...
$<:$, that can be used by the users to weaken specifications for succinctness in
the source.
...
with respect to \term{readOnce}, the equations of which we omit for brevity.
...
and \trule{\textsc{grow}} rules turn the spatial approximation on
...
array variables to \emph{multisets} of index schemes:
%
\begin{align*}
\hspace{-0.5em}
\begin{array}{lr}
...
indices $\overline{e_1}$ are neighbourhood indices.  For
...
to a multiset of index schemes for its subscripts, computed
...
which contain undefined index schemes and adds multiplicity
information:
%
\begin{align*}
...
augmented with multiplicity for the whole set of schemes.
...
dimensionality.  We denote sets of index schemes
...
scheme in the specification there is a scheme in the source model that
...
model of the code in terms of index schemes.
...
\noindent
Each index scheme $u \in U(a)$ is converted to a \emph{unit} span
...
%%
\begin{minted}[linenos=false,breakindent=2.4em,xleftmargin=0.2em]{fortran}
!= stencil centered(depth=1, dim=1)*pointed(dim=2) + centered(depth=1, dim=2)*pointed(dim=1):: a
\end{minted}
%%
...
correctness, and the language of specification is 
more elaborate than ours.
...
``hypertrapezoidal'' algorithms.  Like much of the related work,
