...
model as sets of unifiers for array indexing.

\begin{definition}
  A specification of maximum dimensionality $n$, is modelled by a
  set of unifiers. Here, a unifier is represented as a vector of size $n$
  whose values are drawn from $\mathbb{Z} \cup \{\infty\}$. The purpose
  of the unifier is to accept or reject relativised indices of an array
  subscript. Formal definition of unification in this context is given
  later.

  The element $\infty$ represents the general acceptor meaning it would unify
  with any index in the same dimension including constants or any offset from
  an induction variable.
\end{definition}

\begin{definition} Unification relation, $\sim$, establishes whether
  ...
  Formally, the unification relation can be defined as follows:
  $$u \sim_n v \triangleq
  \{ (u,v) \; \mid \; \forall i \in \{ 1, \ldots, n \}, v_i = \infty \vee u_i = v_i \}$$
\end{definition}

For example, consider a unifier of the form $\vtwoh{\infty}{1}$.

An array index term $(i, j+1)$ would corresponds to the vector
$\vtwoh{0}{1}$ after relativisation. This would unify with the
$\vtwoh{\infty}{1}$ as the first dimension of the unifier have a
general acceptor and the second dimension matches the relative index.

Another index term $(3, j)$ corresponds to the vector $\vtwoh{\infty}{0}$
after relativisation since $3$ is a non-relative index. This would fail
to unify. In the first dimension non-relative index would unify with
the general acceptor but in the second dimension 1 wouldn't unify with 3.
...
specification \textit{spec} in Figure~\ref{fig:spatial-model}. Some
...
Here the meaning of a scaled single-entry vector is that at the
dimension it is not $\infty$, it provides a unifier for consistent
relativised array subscript indices. In particular the constrained
...
Similar to the single-entry vector, zero-entry vector also imposes
a constaint on relativised indices. The only difference is the indexing
expression crosses the origin with respect to the induction variable.
\end{definition}

The first four equations of Figure~\ref{fig:region-model}
give the meanings of the region constants as sets of unifiers.
The next two shows how regions are composed in the model.
...
\begin{alignat*}{2}
& \effdims{M}{n} =
\bigcup_{u \in M} \{x \mid x \in \{1,\ldots,n\}, u_x \neq \infty\} \\
& M \otimes_n N =
  \Bigg\{x \; \Bigg| \;
...
\end{alignat*}
In plain language, tensor product combines non-$\infty$ constraints
...
of an algebra on \textit{specTop} syntax which mirrors the shape
of the syntax with additive and multiplicative binary operators
$\oplus$ and $\odot$ and a constructor for \textit{specTop}:
%%
\begin{align*}
\oplus & : \textit{specTop} \rightarrow \textit{specTop} \rightarrow
  \textit{specTop} \\
\odot & : \textit{specTop} \rightarrow \textit{specTop} \rightarrow
        \textit{specTop} \\
\mathsf{toSpec} & : (\mathbb{Z} \cup \{\infty\}) \rightarrow (\mathbb{Z} \cup
  \{\infty\})  \rightarrow \textit{specTop}
\end{align*}
%%
The operation $\mathsf{toSpec}$ takes a pair of lower and upper bounds
values and constructs a specification which contains a one-dimensional
region. This step of the inference then proceeds by converting each
span into a $n$-dimensional specification by converting each
pair of bounds in a span and multiplying by $\otimes$
then summing the specifications resulting specification by $\oplus$.
That is: 
\begin{equation*}
\textit{spec}(M) = 
\sum\limits_{\vect{x} \in \textsf{regions}(M)}
\prod\limits_{i \in \{1, \ldots, n\}} \, \mathsf{toSpec}(\vect{x}^L_i, \vect{x}^U_i)
\end{equation*}

For each span, and each dimension within it, each pair of lower and
upper bounds is mapped into a specification. That is,
for all $\vect{x}$ of size $n$, and for all $i \in \{1,\ldots,n\}$
then $\mathsf{toSpec} (\vect{x}^L_i, \vect{x}^U_i)$ maps each
pair of lower and upper bounds in each dimension to a piece of syntax 
of \textit{specTop} syntax, defined:
%%
...
&\begin{cases}
%% FWD
 \stenFwdS{\lupper}{i}  & \llower = 0 \, \wedge \, \lupper > 0 \\[0.3em]
 \stenFwdSR{\lupper}{i}{\irreflS}  & \llower = 1 \, \wedge \, \lupper > 0 \\[0.3em]
%% BWD
 \stenBwdS{|\llower|}{i}  & \llower < 0 \, \wedge \, \lupper = 0 \\[0.3em]
\stenBwdSR{|\llower|}{i}{\irreflS} & \llower < 0 \, \wedge \, \lupper = -1 \\[0.3em]
%% CEN
 \stenCenS{\lupper}{i}  & \llower < 0 \, \wedge \, \lupper > 0
 \, \wedge \, |\llower| = \lupper \\[0.3em]
 \stenBwdS{|\llower|}{i} & \llower < 0 \, \wedge \, \lupper > 0  \, \wedge \, |\llower| \neq \lupper \\
\quad \texttt{+} \, \stenFwdS{\lupper}{i}  & \qquad\quad \\[0.3em]
% ABSOLUTE REP
 \epsilon & \llower = \infty \, \vee \, \lupper
 = \infty \\[0.3em]
% BOUND
 \texttt{atMost}, \stenFwdS{\lupper}{i} & \llower > 1 \\[0.3em]
 \texttt{atMost}, \stenBwdS{|\llower|}{i} \quad & \llower < (-1) \\[0.3em]
  \end{cases}
\end{align*}
%%
...
\begin{lemma}
$\mathsf{toSpec}_i$ is total (\ie{}, defined for all spans) given the precondition that the lower
