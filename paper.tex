...
We provide a verification tool for such specifications which can check
...
\item Our specification format is deliberately abstract, with a small
  ...
  %provides full-functionality specifications of stencil computations
  %in the form of a domain-specific language.  
  If a user wishes to specify, say, the
  Naiver-Stokes stencil above, in a deductive verification languages
  like ACSL, they must write a specification which is very
  similar to the code, involving textually dense array-indexing expressions. Therefore any
  low-level mistakes that could be made whilst programming complex indexing code could
...
$(i, j+1)$ can be instantiated (specialised) to $(1, j+1)$. 
...
but which is not used here.% These are used for labelled models, and other components later in.
