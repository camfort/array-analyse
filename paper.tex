...
algorithms for our language (Sect.~\ref{sec:analysis}), derived from
the a denotational model for the language (Sect.~\ref{sec:semantics});
...
\item we perform a quantitative study of
  stencil computations in a corpus of numerical Fortran programs
...
\begin{definition}[Neighbourhood index]
  ...
  variables. We omit $I$ when implicit from the context.
\label{def:neighbour}
\end{definition}
...
%
%
(\texttt{+}\textsc{idem}) \;\; & S \; \texttt{+} \; S \; \equiv \; S \quad
%
...
\begin{definition}[Stencil computations]
  ...
  dimensionalities.  
  A \emph{stencil computation} comprises an iteration by a set of at
  most $n$ induction variables $I$ 
  ...
  that  $\mathsf{schematic}_I(\bar{e}) \neq \bot$ and for all array
  subscripts $b(\bar{e}') \in e_r$ then
...
against specifications (Sect.~\ref{subsec:checking}). These utilise the semantic model of
...
\begin{figure}[t]
\begin{minted}[breakindent=0em,linenos=false,xleftmargin=0em,breakafter=)]{fortran}
...
Our aim is to not only provide a way of exposing errors, but also
for specifying program behaviour to future-proof the code and ease
its maintenance. To increase the likelihood of impact, we 
...
checking procedure on top of CamFort, an open-source analysis
...
simple stencil computations are the norm, for which we can specify a
