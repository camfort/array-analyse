...
\newcommand{\cons}[3]{#2 \;\, \textbf{cons} \,\, #1}
\newcommand{\consA}[3]{#2 \;\, \textbf{cons}_M \,\, #1}

\subsection{Specification checking}

\begin{definition}
Consistency is defined in terms of the binary predicate
$\textbf{cons}$ in Figure~\ref{fig:consistency}.
Consistency relates models of source code and models 
of specifications (which are bounded), thus
 $\textbf{cons} \subseteq \mathcal{P}(\domainVal{}^n)
\times \textsf{Approx}(\mathcal{P}(\domainVal{}^n))$. 
We denote sets of indexing schemes $\mathcal{P}(\domainVal{}^n)$
that come from a specification model by $S$ and from source code model
by $C$.
...
\begin{array}{c}
%%
\trule{up}\dfrac{\forall u \in C \, \exists v \in S .  \, u \preceq v}
      {\cons{\textsf{upper}(S)}{C}{n}}
\qquad
\trule{low}\dfrac{\forall v \in S \, \exists u \in C . \, u \preceq v}
      {\cons{\textsf{lower}(S)}{C}{n}} \\[1.5em]
%%
\trule{exact}\dfrac{\cons{\textsf{lower}(S)}{C}{n} \qquad \cons{\textsf{upper}(S)}{C}{n}}
  {\cons{\textsf{exact}(S)}{C}{n}} \\[1.5em]
%%
\trule{both}\dfrac{\cons{\textsf{lower}(S_1)}{C}{n} \qquad \cons{\textsf{upper}(S_2)}{C}{n}}
  {\cons{\textsf{both}(S_1, S_2)}{C}{n}}\\[1.5em]
%%
\trule{$1$}
\dfrac{\cons{S}{C}{n}}{\consA{\textsf{once}(S)}{\textsf{once}(C)}{n}}
\;\;
\trule{$*$}
\dfrac{\cons{S}{C}{n}}{\consA{\textsf{mult}(S)}{\textsf{mult}(C)}{n}}
  \\[1.5em]
\trule{$1 < *$}
\dfrac{\cons{S}{C}{n}}{\consA{\textsf{mult}(S)}{\textsf{once}(C)}{n}}
\end{array}
\end{align*}
