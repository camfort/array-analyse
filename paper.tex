...
wildcard.
Throughout $u, v, w$ will range over index schemes.
\end{definition}
...
the models of subterms. The model of $\term{*}$ is uses
an intermediate tensor $\otimes$ which requires some intermediate
definitions.

\begin{definition}[Constrained dimensions]
Given an $n$-dimensional model $M$, the \emph{constrained dimensions}
of that model is a subset of $\{1, \ldots, n\}$ giving
...
\end{alignat*}
The intuition for the tensor is that it gives all possible
ways of combining indexing schemes in two models
such that for every scheme
$u \in (M \otimes_n N)$ then $w \sim u$ for some $w$
\emph{iff} $\;\exists v \in M . w \sim v$ and
$\exists v' \in N . w \sim v'$. Thus it acts as a kind of intersection
for indexing schemes. We unpack its definition in pieces.

We start with the first guard taking the Cartesian product of
the two models and quantifying over all pairings $(u, v)$. 
The second guard says that $j$ is in the set of constrainted
dimensions for $M$ and the constrained dimensions for $N$.
That is, every $j$ corresponds to a dimension in which 
either $M$ or $N$ has a scheme $u$ where $u_j \neq \infty$,
and thus is constraining. 

%for all possible
%pairs of schemes $(u, v)$ in the Cartesian product of the
%two models $M \times N$, then 
