...
following rule $R_1 + R_2 \equiv R_3$ which we denote: $\term{fwd + fwd} \equiv \term{fwd}$, $\term{bwd + bwd} \equiv \term{bwd}$, 
...
A piece of code consistent with a specification $S$
 is consistent with a generalisation $T$ of the specification.
That is for all models $M$ of a piece of code:
\begin{equation*}
S <: T \; \wedge (\consA{M}{\interp{S}_n}) \;
\Rightarrow \; (\consA{M}{\interp{T}_n})
\end{equation*}
where $\consAName$ is the consistency predicate used in checking (\S~\ref{subsec:checking}).
\end{theorem}
