...
& \mathit{modelise} (\textit{assg}, x) =
  ...
  \end{cases}
\end{align*}
Thus, checking proceeds by reusing the \textit{analyse} function of
inference (Section~\ref{subsec:inference}) to extract indexing
schemes for stencil computations flowing to an assignment. 
...
stencil specification inferred previously in Section~\ref{subsec:inference}:
%
