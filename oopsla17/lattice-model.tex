...
\begin{defn}{(Extended origin)}
  In $N$-dimensions, the \emph{extended origin} is the interval
  $\interv{-1}{1}{N}$ contained in \region{N}.
\end{defn}
...
\begin{defn}{(Extended origin distance)}
  ...
  This notion extends to sets of points. Since an there may be an infinite
  number of points, we measure the distance from the closest one.

  \todo{Mistral to check: I may have broken this notion}
\end{defn}
%
\begin{prop}{}
  The closed form of extend origin distance for an $N$-dimensional subscript $S$
  is denoted with $|S|$ and is defined as:
%
  \begin{align*}
    |S| & = \sum_{i=1}^{N} dist_i(S)
    &
    \textit{dist}_i(S) & = \begin{cases}
...
\begin{proof}
  \todo{%
    Not sure what we are going to prove here? That it is a metric space or
    something?

    Response: The definition says extended origin distance is the smallest
    number of translations to reach the extended origin, we need to prove that
    this formula indeed gives that.
  }
\end{proof}

\todo{%
  We used to call the below property contiguity, and I think this is a natural
  notion- does ``close neighbour'' have some standard use or is it okay to call
  this contiguity?

  Response: how about adjecency?
}
\begin{defn}{(Adjacent)}
  ...
  at $j$ then $\pi_j(S) \neq \pi_i(S')$ such that $|S| = |S'| + 1$.

\todo{%
  Not sure how to rewrite the following as I have a bit of hard time parsing it.
}
