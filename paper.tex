...
each $v \in \mathsf{dom}(M)$, the next step of the algorithm coalesces
the set of relative index vectors $M(v)$ into contiguous regions within an
$n$-dimensional space.
...
\end{align*}
%%
\item Fold each list pairwise by the following partial $\bullet$ which 
coalescees and contiguous regions:
%
\begin{align*}
...
\end{align*}
%
\item Filter by $\containedin$, defined: 
%
\begin{align*}
