...
\newcommand{\stenCen}[2]{\texttt{centered} \, (\texttt{depth=}#1,
  \texttt{dim=}#2)}
\newcommand{\stenFwdS}[2]{\texttt{fwd} \, (\texttt{depth=}#1,
  \texttt{dim=}#2)}
\newcommand{\stenBwdS}[2]{\texttt{bwd} \, (\texttt{depth=}#1,
  \texttt{dim=}#2)}
\newcommand{\stenCenS}[2]{\texttt{cen} \, (\texttt{depth=}#1,
  \texttt{dim=}#2)}
...
\nonterm{region} ::= & \term{forward}, \term{depth=}\mathbb{N}_{>0}, \term{dim=}\dimTy  \\
\mid \; & \term{backward}, \term{depth=}\mathbb{N}_{>0}, \term{dim=}\dimTy \\
\mid \; & \term{centered}, \term{depth=}\mathbb{N}_{>0}, \term{dim=}\dimTy \\
\mid \; & \nonterm{region} \; \term{+} \; \nonterm{region} \\
\mid \; & \nonterm{region} \; \term{*} \; \nonterm{region} \\
\mid \; & \nonterm{rvar}  \\[0.5em]
...
\end{array} \\ \\
%
\framebox{$\overline{\textit{specDec} :: \texttt{v}} \equiv 
\overline{\textit{specDec'} :: \texttt{v}}$} \\[1em]
\begin{array}{rl}
(\textsc{mutual}) \; & 
