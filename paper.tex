...
\begin{abstract}
  ...
  specification and program synthesis procedures. We evaluate our
  ...
  checking, and program synthesis procedures, our implementation, and
...
symmetrical access pattern (``centered'') to a depth of one in each
...
dimension. Figure~\ref{ref:navier-stokes-fragment}(c) illustrates
the specifications pictorially.

In this paper, we make the following contributions:
%
\begin{itemize}
\item we introduce our specification language for
stencil computations that captures many common forms
of data access pattern (Section~\ref{sec:lang})
along with a sound denotational model (Section~\ref{sec:semantics});

\item we detail inference, checking, specification synthesis,
and program synthesis algorithms for our language
(Section~\ref{sec:analysis}), which relies on the semantic model;

\item we evaluate our approach, providing an
implementation as an extension of CamFort,
an open-source program analysis and refactoring tool for Fortran
corpus. We use this implementation to evaluate our approach against
a corpus of scientific applications, totalling roughly one million
lines of code, and we study some verification case studies;

\item we show that around 10\% of stencils are in a class that
  represent a plausible scenario for a programming error;

\item we consider two particular stencils in detail and show that we
...
duvdy = ((v(i,j)+v(i+1,j))*(u(i,j)+u(i,j+1))+ &
  ...
  gamma*abs(v(i,j-1)+v(i+1,j-1))*(u(i,j-1)-u(i,j))) & /(4.0*dely)
...
Our specification language is an approach in this direction, proving
...
Much of this paper concerns the technical aspects of the language,
its semantics, its inference, checking, and synthesis procedures.
The evaluation focuses on its application to real scientific code base
...
Section~\ref{sec:syntax} outlines the syntax of specifications.
 Section~\ref{sec:eqs} provides an equational
theory for specifications via relation ($\equiv$) and a notion of
sub-specification via relation ($<:$).  Section~\ref{sec:semantics}
...
only within the scope of the loop. Induction variables are ranged
over by $i$, $j$, $k$ throughout. Since we do not use non-base
(derived) induction variables we often simply say \emph{induction variable}.
\end{definition}
...
Let $U$ range over the finite maps generated by \textsf{analyse}.  For
each $x \in \mathsf{dom}(U)$, the next step of the algorithm
constructs a set of $n$-dimensional rectangles covering all contiguous groups of schemes
in $U(x)$. 
...
\begin{definition}[Spans]
  ...
  a pair of $n$-dimensional vectors (or represented as a $2 \times n$
  matrix) with values drawn from $\domainVal{}$. This  
   gives the co-ordinates of a lower-bound vertex (first
...
In the rest, $\vect{x}, \vect{y}$ range over spans. 
For a span $\vect{x}$, we write $\vect{x}_{i:j}$ for the sub-span
comprises the subvectors $[\vect{x}^L_{i:j} \; \vect{x}^U_{i:j}]$ from entry $i$ to
entry $j$ (inclusive).
\end{definition}

\noindent
The indexing schemes of $U(x)$ are converted to \emph{unit} spans $u \mapsto [u \; 
...
% taking permustations is a systematic way of producing all rotations
% of the n-dimensional space - forward ref to fold
The next step of the algorithm repeatedly coalesces any contiguous spans
until a fixed point is reached, resulting in a set of spans covering
over the original space of schemes. Two $n$-dimensional spans $\vect{x}$ and $\vect{y}$
are contiguous if for all but one $j \in \{1, \ldots, n\}$ then
$\vect{x}_j = \vect{y}_j$ and for one $j$ then $\vect{x}^U_1 + 1 =
\vect{y}^U$. For example, unit spans
$\left[\vtwoh{-1}{0}\; \vtwoh{-1}{0}\right]$ and $[\vtwoh{0}{0} \; \vtwoh{0}{0}]$ 
are contiguous in the first dimension and can be coalesced
into the span $[\vtwoh{-1}{0} \; \vtwoh{0}{0}]$.

\begin{definition}\label{def:span-coalesc} Our operator for span
...
%%
\end{definition}
In order to coalesce regions which are contiguous in other dimensions
other than the first, we generate all rotations of the spans in
$n$-dimensional space by permutation, and fold together contiguous
regions using the above $\bullet$ operation. For our example,
from the unit spans in Figure~\ref{fig:inference-steps-informal}(a)
the contiguous spans in Figure~\ref{fig:inference-steps-informal}(b)
are generated. Then any spans that are contained with any other span are deleted,
...
are used to synthesis concrete stencil specification syntax, where
each span is product \texttt{*} of regions and multiple spans are
combined with the region sum \texttt{+}.

More formally, the algorithm to create contiguous spans, covering the space of
indices, now proceeds as follows.
Firstly, for all $v \in \mathsf{dom}(M)$ a new map $M'$ is
...
\end{align*}
%%
A fixed point is then computed for the following
procedure \spanOp which, per variable
...
is indeed in DNF-form, where $\odot$ is essentially applying
 the \trule{\textsc{dist}} rule of our equational theory
 (Figure~\ref{fig:equations}). The operation is
...
Checking procedure synthesises syntactic specification into its model and
generates an injection from indexing schemes to the multiplicity of observed
access patterns, this is formally defined in Figure~\ref{fig:consistency}.
Then, the consistency of the injection from the indexing schemes with the model
is checked.

It the multiplicity of the access pattern and the model is matched and are
$\mathit{once}$, then the problem is reduced to checking consistency in
models in which duplicate access patterns are allowed.

Next, a consistency predicate considers whether the stencil is an approximation.
...
%
\begin{enumerate}
\item From now on assignment on line three is referred as $L3$ for brevity.
  Verification is done by computing the following:
  $$check(\texttt{stencil readOnce, fivepoint},L3,b)$$
%
\item Generate model from the specification as described in
  Section~\ref{sec:semantics}. We refer to the indexing
  schemes within the model as set $m$.
%
\[ m= \{  \vtwo{-1}{\infty},
          \vtwo{0}{\infty},
          \vtwo{1}{\infty},
...
\interp{\texttt{stencil readOnce, fivepoint}}_2 =
  \{\textsf{once}(\textsf{exact}(m)) \}
\end{dmath*}
%
\item To evaluate the two predicates above, we need indexing schemes for the
  array in question. This is obtained by $modelise$ function. For brevity the
  set of indexing schemes is referred as $m'$.

  For the example above the observed model is as follows:
%
\[
m' = \{
    \vtwo{0}{-1},
  ...
  \}
\]
\[modelise(L3,b) = \textsf{once}(m')\]
%
\item Now $cons$ predicate is applied with this model along side with the
  assignment on line three and array $b$.

  The predicate is reduced to
  $cons(\textsf{once}(\textsf{exact}(m)),\textsf{mult}(m'))$.
%
\item $cons$ predicate from the previous step evaluates to
  $$cons(\textsf{mult}(\textsf{exact}(m)),\textsf{once}(m')).$$
%
\item We further evaluate the predicate to
  \begin{dmath*}
    cons(\textsf{mult}(\textsf{up}(m)),\textsf{mult}(m')) \wedge
    cons(\textsf{mult}(\textsf{low}(m),\textsf{mult}(m')).
  \end{dmath*}
%
\item Below we present two sets of generalisations to show both the lower and
  ...
  three, the stencil computation is consistent with the \texttt{fivepoint}
  specification.
\end{enumerate}

Now when $L3$ is modified with an additional term \texttt{b(i+2,j+2)},
...
\end{minted}
%
In the first six steps, evaluation proceeds the same way except for step three.
In the third step, result of $\mathit{modelise}(L3',b)$ will have the extra term
$\vtwoh{2}{2}$. In the seventh step, since the model has not changed and the
indexing schemes are only augmented with a new element, the generalisations for
lower bound still hold. However, there are no elements in the model, $m$, that
can generalise $\vtwoh{2}{2}$, hence the model fails to hold as an upper bound.
...
$\mathit{cons}(\textsf{mult}(\textsf{up}(m)),\textsf{mult}(m'))$ would not have
been needed. This semantically makes sense since the region $\texttt{fivepoint}$
is indeed contained in the regions touched by $b$.

\subsubsection{A note on stencil approximations}

As noted above, the \textsf{fivepoint} region defined above is more accepting
than that used in Section~\ref{sec:lang}. This introduces a new notion of
approximation beyond that is achieved in Section~\ref{sec:semantics}
using upper and lower bounds because the in both cases the specifications are
used to generate \emph{exact} models. Formally, we say a specification $s'$
approximates a more specific specification $s$ \emph{iff} $\forall m \in
\textsf{Mult}(\domainVal) . \mathit{cons}(s,m) \implies \mathit{cons}(s',m)$.

Equations provided on regions in Section~\ref{sec:lang} when all modifiers
equal provide such approximations, but it goes further than that. The
regions describing the fivepoint stencil for example cannot be shown equal
using these equations, but satisfies the approximation predicate as defined
above.

The checking algorithm though happily accepts an approximation in place of the
most specific specification. Although this decreases the likelihood of catching
...
We evaluated the stencil language on a corpus of scientific code. We first give an overview of the corpus itself before presenting our results.
...
\textbf{Navier} is a small project, giving a discrete
...
eventually generate some stencil specifications is also counted as
...
induction variable (\mintinline{fortran}{i}) as in \mintinline{fortran}{x
...
We grouped stencils together into four classes based on similar levels of complexity. These four classes account for 99.4\% of all the stencils we found in the corpus.

\textbf{All-reflexive} The vast majority (66\%) of stencils were
...
\textbf{Some-reflexive} stencils (24\%) consist of only reflexive terms but
leave some dimensions unspecified. Here are two examples:
\begin{minted}{fortran}
...
\textbf{Single-action} stencils (6\%) consist of a single \term{backward},
\term{forward}, or \term{centered} region constant and some number of \term{reflexive}
region constants.
...
\textbf{Single-action-irreflexive} stencils (3\%) ) consist of a single
\term{backward}, \term{forward}, or \term{centered} irreflexive region constant
and some number of reflexive region constants.
...
stencil. Stencils of this type required a neightbourhood index without a zero-offset in one dimension only. The remaining dimensions must either have no offset or not pertain to an induction variable.

The remaining stencil shapes (less than 1\%) consisted primarily of multiple \term{forward}, \term{backward}, or \term{centered} dimensions. We note that only 0.2\% of stencils found required use of the union (\term{+}) operator. Despite this low frequency these stencils are arguably the most important since they represent more complicated code and thus greater potential for programming errors.
...
\begin{figure}[t]
  ...
  \caption{Greyscale output of several perturbations of a Jacobi
    ...
    \subref{fig:jacobicorrect}.}
  \label{fig:jacobi}
\end{figure}

\subsubsection{Turbulence calculation}
...
\begin{minted}[breakindent=0em,linenos=false,xleftmargin=0em]{fortran}
