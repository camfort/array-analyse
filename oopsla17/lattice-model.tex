...
\begin{defn}{(\emph{Points})}
  A \emph{point} is a Cartesian product (vector) of $\zinf$ values representing an
  $N$-dimensional point. The set of $N$-dimensional offsets is
  written $\zinf^N$. We denote sets of $N$-dimensionl points
by the set $\mathcal{P}(\zinf^N)$.

A set of $N$-dimensional points \emph{projected} into the set
of 1-dimensional points in the $i^{\text{th}}$ dimension by $\pi_i : \mathcal{P}(\zinf{})^{\mkern-19mu{N}} \to
  \mathcal{P}(\zinf{})$ where $i \in \mathsf{Fin}(N)$. We refer to this projected set as the \emph{$i^{th}$ components}.
\end{defn}
...
\vee
\;\;
\pi_i (X) = \mathbb{Z} = \interv{-\infty}{\infty}{\mathsf{true}}
\end{equation*}
%
...
interval vector $[-1, 1]^N$ which is a subspace of \region{N}. 
\end{defn}
