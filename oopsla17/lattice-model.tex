...
\begin{defn}{(Extended origin)}
  ...
  $\interv{-1}{1}{N}$ which is a subset of \region{N}.
\end{defn}
...
\begin{defn}{(Close neighbour)}
  A set of points $P$ is said to be a \emph{close neighbour at index $j$} of a
  set $P'$ if for every index $i$ except $j$, $\pi_i(P) = \pi_i(P')$ but at $j$
  then $\pi_j(P) \neq \pi_i(P')$ and $\textit{dist}_j(P) = \textit{dist}_i(P') +
  1$.
