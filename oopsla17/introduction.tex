...
also a lack of tools targeted at the needs of scientists. We aim to
change that. This
paper is part of a line of research providing lightweight, easy-to-use
...
\end{minted}
%
This is a \emph{stencil} computation, an idiom
where elements of an array at each index $i$ are computed from a
\emph{neighbourhood} of values around $i$ in some input array(s). Stencils
are common in scientific, graphical, and numerical
...
as writing $\texttt{a(i)}$ instead of $\texttt{a(i-1)}$ (we revisit
examples we found of this in \Cref{sec:case-studies}).
...
contiguous elements with simple static patterns. From this we
designed a simple set of combinators to express common array patterns (\Cref{sec:lang}).
As an example, the one-dimensional Laplace program is specified in our language by:
%
...
abstract to avoid the error-prone nature of index manipulation and
are lightweight to aid their adoption by scientists.
...
corresponding to code that has a higher potential for errors. This
...
We use the following terminology for syntactic constructs in our target language.
...
\begin{defn}[Array subscripts and indices]\label{def:array-subs}
  ...
  which is constant with respect to the enclosing loop.
\end{defn}
...
\begin{defn}[Neighbourhood index]\label{def:neighbour-ix}
  ...
  \emph{neighbourhood index} (or \emph{neighbour}) if $e$ is of the
  form $e \equiv i$, $e \equiv i + c$, or $e \equiv i - c$, where $c$
  is an integer constant. That is, a neighbourhood index is a constant
  translation of an induction variable. (The relation $\equiv$
  identifies terms up-to commutativity of $+$ and the inverse
