...
lines)~\cite{wheeler2001sloccount}; including them our corpus
totals
...
\begin{defn}[Index scheme]
  ...
  extended integer sets, \ie{}, a member of $\mathcal{P}(\zinf)^{N}$.
  An alternate (equivalent) view is of a $N$-times finite Cartesian
  product on subsets of $\zinf$.  We use $S, T, U$ to denote index
  schemes.
...
%\begin{theorem}
%$N$-dimensional index schemes form a lattice where join and m
%\end{theorem}

\begin{lem}{}\label{lem:vector-intersect}
  Intersection distributes over $N$-dimensional index schemes. 
  That is, for index schemes $I, J \in \mathcal{P}(\zinf)^N$ then:
%
  \begin{equation*}
    I \cap J = \prod_{i = 1}^{N} \pi_i(I) \cap \pi_i(J)
  \end{equation*}
\end{lem}
...
Let $I, J$ be $N$-dimensional index schemes such that
$\pi_i(I) = \pi_i(J)$ for all $1 \leq i \leq N$ apart 
from some dimension $k$ where $\pi_k(I) \neq \pi_k(J)$,
then:
%
  \begin{equation*}
    I \; \cup \; J
    =
    \pi_1(I) \times \cdots \times
    (\pi_k(I) \cup \pi_k(J)) \times \cdots \times
    \pi_N(I)
  \end{equation*}
%
\end{lem}
...
We define an extended notion of closed interval on 
