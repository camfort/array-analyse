...
%\\[1em]
\nonterm{specTop} ::= & [\nonterm{approxMod},] \; \nonterm{spec} \\
\nonterm{spec} ::= & [\nonterm{modR}] \; \nonterm{qualifiedRegion} \\
\nonterm{qualifiedRegion} ::= & [\nonterm{modRefl},] \; [\nonterm{modIrrefl},] \; \nonterm{region}\\
\nonterm{modR} ::= & \term{readOnce} \\
...
\begin{align*}
M \otimes N & =
  \{(u \bowtie v)_i \mid i \in \{1, \ldots, 2^n\}, (u, v) \in M \times N\}
\end{align*}
...
$i - 1$. Recall that the vectors are defined over the domain
$\mathbb{Z} \cup \{\infty\}$.
