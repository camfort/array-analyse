...
Now we informally describe what it means for a stencil kernel to be consistent
with a specification. This will will be formalised in \S~\ref{sec:semantics} and
algorithmically described in  and \S~\ref{sec:analysis}.
...
crossing the origin with respect to the induction variable. Consistency is then
a two way requirement. All array subscripts on $a$
contributing to the kernel should be consistent with the region. Conversely,
all parts of the region should have an index corresponding to that part.
...
Figure~\ref{fig:syntax} gives the syntax of stencil specifications, which is
...
region declarations. For example, the last specification
 example above can be restated as:
%%
\begin{minted}{fortran}
!= region :: r1 = centered(depth=1, dim=1) * pointed(dim=2)
!= region :: r2 = centered(depth=1, dim=2) * pointed(dim=1)
!= region :: fivePoint = r1 + r2
!= stencil fivePoint :: a
\end{minted}
This is especially useful for common stencil patterns, such as five point
stencil, as the region can be defined once and reused many times later.
%%
\paragraph{Modifiers}
%%
...
\begin{definition}
  ...
  form $i \pm c$, Def.~\ref{def:neighbour}) and $\infty$ is a wild-card
  representing both constant indices and unspecified indices. Throughout, $u, v,
  w$ range over
  index schemes and $u_i$ denotes the $i^{th}$ element of $u$. We
  write $(\domainVal)^n$ for the set of all $n$-dimensional schemes.
\end{definition}

Indexing schemes give an abstracts array subscripts.
...
For brevity we use abbreviations $\term{fwd}$, $\term{bwd}$, and
$\term{cen}$ for region constants here.

The equality relation $\equiv$ is defined over \emph{region}
syntax which gives the following axioms on regions:
%
\begin{itemize}
...
akin to Disjunctive Normal Form (\S~\ref{sec:inf-step4}).
...
\end{equation*}
where $\consAName$ implements consistency in checking (\S~\ref{subsec:checking}).
\end{theorem}
...
with \textsf{schematic} (Def.~\ref{def:schematic}).
...
We illustrate each step of inference using the
 five-point stencil program from Example~\ref{exm:checking}.
...
(Def.~\ref{def:span-coalesc}) to coalesce contiguous regions,
...
Def.~\ref{def:mult-and-approx}).  The algebra mirrors the shape
...
syntax (see Figure~\ref{fig:syntax}).
...
is indeed in DNF-form, where $\hat{\odot}$ is essentially applying
 ...
 (Fig.~\ref{fig:equations}). The operation is
